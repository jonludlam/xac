...
function processrrd(parent,host,exp) {
    var rows=exp.meta.rows;
    var columns=exp.meta.columns;

    if(rows==0)
	return parent.hoststats[host].rings["t"].peek(0); // last update time
    ...
    var myrrd=parent.hoststats[host];
    if(!myrrd) {
	myrrd=new rrd(["t"].concat(exp.meta.legend),data);
	parent.hoststats[host]=myrrd;
    } else {
